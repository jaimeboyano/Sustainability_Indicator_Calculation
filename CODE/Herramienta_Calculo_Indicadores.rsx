...
romano_a_arabigo <- function(romano) {
  # Tabla de correspondencia entre valores romanos y arábigos
  valores_romanos <- c(1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1)
  letras_romanos <- c("M", "CM", "D", "CD", "C", "XC", "L", "XL", "X", "IX", "V", "IV", "I")
  # Inicializar valor
  valor_arabigo <- 0
  ...
  while (nchar(romano) > 0) {
    ...
    if (substr(romano, 1, 2) %in% letras_romanos) {
    ...
    if (letter %in% letras_romanos) {
      valor_arabigo <- valor_arabigo + valores_romanos[which(letras_romanos == letter)]
    }
  }
...
if ('2. Compacidad Absoluta' %in% Indicadores_a_calcular) {
  if (!is.null(Constru_interseccion)) { 
    ...
    Constru_interseccion$CONSTRU_Edit <- gsub("[.?]", "", Constru_interseccion$CONSTRU_Edit)
    Constru_interseccion$CONSTRU_Edit_Arab <- as.numeric(lapply(Constru_interseccion$CONSTRU_Edit, romano_a_arabigo))
