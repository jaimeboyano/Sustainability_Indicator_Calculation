...
##Buildings_Pop_Raster=output raster
##Pop_Tot_Raster=output raster
...
if ('6.6 Proximidad a los 5 servicios' %in% Indicadores_a_calcular) {
    Facilities_600 <- subset(Clasificacion_Intermedia_Clasificador_Catastral, USO == "EQUIP_OTR" | USO == "OCIO_ESP" | USO == "EQUIP_EDU" | USO == "EQUIP_SANI")
    Bufer_Facilites_600 <- st_as_sf(st_union(st_buffer(Facilities_600, 600)))
    Facilities_300 <- subset(Clasificacion_Intermedia_Clasificador_Catastral, USO == "COM")
    Bufer_Facilites_300 <- st_as_sf(st_union(st_buffer(Facilities_300, 300)))
    Bufer_tot <- st_intersection(Bufer_Facilites_600, Bufer_Facilites_300)
    Residential_Intersects_Bufer <- st_intersection(Buildings_Join_Sections, Bufer_tot)
    Pop_Tot_Raster <- poligons_to_raster_200(Residential_Intersects_Bufer, Buildings_Pop_Raster, 'Building_Pop', 'sum')
    Porc_Tot <- Pop_Tot_Raster/Buildings_Pop_Raster*100
