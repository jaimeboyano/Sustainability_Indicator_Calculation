...
Para desarrollar este estudio se han utilizado herramientas de programación y Sistemas de Información Geográfica. Estos instrumentos incluyeron el lenguaje de programación R y su entorno integrado de desarrollo (IDE) Rstudio, así como los paquetes CatastRo (Delgado Panadero y Hernangómez, 2023), Sf (Pebesma, 2018) y raster (Hijmans, 2023), el Sistema de Información Geográfica Qgis y junto con los plugins Cadastral Classifier Classifier (Shurupov et al., 2023) y QGIS Processing R Provider (North Road & Caha, 2019). 
### 4.1 Paquetes de Rstudio
R es un lenguaje de programación diseñado para el análisis estadístico. Su naturaleza de código abierto permite la incorporación de numerosos de paquetes y funcionalidades. Estos módulos contienen un conjunto de funciones que, el código fuente y manuales de ayuda sobre su uso (Royé y Serrano Notivoli, 2019). En este estudio se han utilizado las siguientes extensiones de R. 
El paquete CatastRo consiste en una serie de funciones y herramientas para trabajar con los datos catastrales públicos de la directiva INSPIRE relacionados con la Sede Electrónica del Catastro. Para ello, el servicio de consulta establece tres tipos de consultas: el servicio ATOM (permite la descarga de lotes vectoriales, utilizado en este estudio), el servicio WFS (permite descargar objetos vectoriales de elementos específicos) y el servicio WMS (permite descargar imágenes georreferenciadas de los elementos catastrales.
El paquete “sf” es una herramienta innovadora para la gestión y manipulación de datos geoespaciales. Aunque comparte algunas funcionalidades con las extensiones anteriores “sp” y “rgdal”, “sf” introduce nuevas funcionalidades y una sintaxis más intuitiva, utilizando estructuras de datos nativas de R. Sus características clave incluyen una estructura de datos simplificada que trabaja con objetos tipo dataframe o tibble, una sintaxis intuitiva con funciones que comienzan con “st_” y se pueden concatenar con el operador pipe (%>%), una fácil conexión con bases de datos PostGIS, cumplimiento con los estándares geoespaciales Open Geoespacial Consortiuem (OGC) y el estándar USO 19125 y la integración de librerías clave como GDAL, GEOS y PROJ para tener un rendimiento óptimo y amplias capacidades de análisis de datos geoespaciales (Royé & Serrano Notivoli, 2019).
Por último, el paquete “raster” permite “leer, escribir, manipular, analizar y modelizar datos espaciales en formato ráster” (Hijmans, 2023). Este complemento es muy popular y se utiliza para procesar grandes conjuntos de datos que no pueden cargarse en la memoria del ordenador. Trabajar por lotes, es decir, lee, calcula y escribe diferentes bloques de datos sin cargar todos los datos a la vez, lo que reduce el uso de memoria en el ordenador. En las versiones más recientes, este módulo ha sido sustituido por “terra” que además incorpora el tratamiento de datos vectoriales y permite el procesamiento de archivos de mayor tamaño, siendo s más intuitivo de utilizar. 
## 4.2 Plugins de QGIS
QGIS es un software SIG de código abierto, lo que implica que es libre y accesible para toda la comunidad. Una de las ventajas de QGIS como software libre es la posibilidad de utilizar, modificar y distribuir tanto el programa principal como los complementos o plugins. Los plugins son diferentes herramientas adicionales que se pueden agregar a QGIS para ampliar sus funcionalidades y adaptarlo a necesidades específicas. Estos plugins son desarrollados tanto por la comunidad de usuarios como por desarrolladores profesionales, garantizando una gran cantidad de opciones. La posibilidad de utilizar y compartir plugins fomenta el intercambio de información y conocimientos entre diferentes usuarios, enriqueciendo la experiencia y las capacidades del software. En este trabajo se han utilizado los siguientes complementos. 
El complemento Clasificador Catastral (Cadastral Classifier) permite clasificar el parcelario urbano de cualquier municipio de España (salvo Navarra y País Vasco), según información del catastro. Su objetivo es el de ofrecer, en función de las necesidades del usuario, diferentes clasificaciones de parcelas de los municipios según los tipos de edificación en cada una de ellas. Estas clasificaciones tienen diferentes niveles de sistematización y profundidad. Además, consta de dos módulos: el primero de ellos dedicado al tratamiento de datos y el de clasificación. El primero de ellos combina la información alfanumérica (en archivos CAT) con la cartografía vectorial y permite la ejecución para un único municipio o para varios. Ofreciendo tanto la unión de la tabla de registro de tipo 14 como del resto. Mientras que el módulo de clasificación permite seleccionar entre tres tipos de clasificación: la clasificación básica (7 categorías), la clasificación intermedia (14 categorías pudiendo establecer un umbral para definir las clases de residencia unifamiliar, plurifamiliar, mixto y edificación) y la clasificación avanzada que permite un mayor control para definir las clases según el porcentaje de cobertura de cada uso de suelo. Estas clasificaciones se basan en la columna de la tabla 14 llamada “Tipología constructiva (Normas Técnicas de Valoración)” (Shurupov et al., 2023). 
